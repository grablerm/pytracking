...
def run(settings):
    ...
    print("Training daatset done")

    # Validation datasets
    coco_val = MSCOCOSeq(settings.env.coco_dir)
    ...
    proposal_params = {'min_iou': 0.1, 'boxes_per_frame': 22, 'sigma_factor': [0.01, 0.05, 0.1, 0.2, 0.3]}
    ...
    data_processing_val = processing.ATOMProcessing(search_area_factor=settings.search_area_factor,
                                                    ...
                                                    joint_transform=transform_joint)
    print("Data processing on validation pairs")
    ...
    dataset_val = sampler.ATOMSampler([coco_val], [1], samples_per_epoch=500*settings.batch_size, max_gap=50,
